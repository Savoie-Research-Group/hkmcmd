...
def main(argv):
    ...
    data_time = np.empty((args.total_steps,), dtype=float)
    data_voxels = np.empty((args.total_steps, len(atoms_list)), dtype=int)
    data_types = np.empty((args.total_steps, len(atoms_list)), dtype=int)
    ...
    for step in range(1, args.total_steps):
        ...
        for molecule in molecules_list:
            molecule.assign_voxel_idx(voxels_datafile)
        data_time[step] = data_time[step - 1] + dt
        for molecule in molecules_list:
            for atom in molecule.atoms:
                atom_idx = atom.ID - 1  # assuming atom IDs start from 1
        ...
        if step % args.print_every == 0:
            print(f"\nStep {step} ({datetime.datetime.now()})")
            print(f"Event: {events_types[event_idx]}, Extra: {events_extra[event_idx]}")
    ...
    df.to_csv(f"{args.prefix}.lkmc_output.csv", index=False)
    voxel_centers = {idx: center for idx, center in enumerate(voxels_datafile.centers)}
    np.save(f"{args.prefix}.lkmc.voxel_centers.npy", voxel_centers)
    #vc = np.load(f"{args.prefix}.lkmc.voxel_centers.npy", allow_pickle=True).item()
    #print(vc)
